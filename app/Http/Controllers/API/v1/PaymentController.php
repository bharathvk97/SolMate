...
class PaymentController extends Controller
{
    ...
    private function getRazorpayInstance()
    {
        ...
        // Fail clearly if the Razorpay keys are missing or still the placeholder
        // values from .env.example, instead of throwing an opaque gateway error.
        if (empty($keyId) || empty($keySecret)
            || str_contains($keyId, 'XXX') || str_contains($keySecret, 'XXX')) {
            abort(response()->json([
                'status'  => false,
                'message' => 'Online payments are not set up yet. Please add your Razorpay API keys to continue.',
            ], 503));
        }
