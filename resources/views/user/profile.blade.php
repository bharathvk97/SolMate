    ...
    <div class="col-md-4 text-center">
      <div class="card-findr p-3">
        <div class="profile-avatar-wrap mb-2">
          <img src="{{ $user->avatar_url }}" alt="{{ $user->name }}" style="width:100px;height:100px;border-radius:50%;object-fit:cover;border:3px solid var(--brand-primary);">
        ...
        <p style="font-size:.8rem;color:var(--text-muted);margin:3px 0;">{{ ucfirst(str_replace('_',' ',$user->role)) }}</p>
        <span class="badge-status badge-{{ $user->status }}" style="margin-top:.5rem;display:inline-block;">{{ ucfirst($user->status) }}</span>

        @if(in_array($user->role,['hostel_owner','mess_owner']))
        ...
        <span style="background:rgba(16,185,129,.1);color:var(--brand-accent);border-radius:20px;padding:4px 12px;font-size:.78rem;font-weight:700;">
          ✓ Active until {{ $user->subscription_expires_at->format('d M Y') }}
        </span>
        @else
        ...
        <a href="{{ route('owner.subscription') }}" class="btn-primary-findr d-block mt-2" style="font-size:.78rem;padding:.4rem;">Renew</a>
        @endif
        @endif
      </div>
    </div>
        ...
        <form method="POST" action="/api/v1/profile" id="profileForm" onsubmit="saveProfile(event)">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="tel" name="phone" class="form-control" value="{{ ltrim($user->phone,'91') }}" maxlength="10">
            </div>
          </div>
